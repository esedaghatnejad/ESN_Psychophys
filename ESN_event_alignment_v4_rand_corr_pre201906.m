function ESN_event_alignment_v4_rand_corr_pre201906
%% clear
clear;
...
%% load continuous file
[file_name,file_path] = uigetfile([EPHYS.file_path_CH_EVE filesep '*.continuous'], 'Select any continuous file');
fprintf(['Loading ', file_name, ' ... ']);
if ~strcmp(file_path(end), filesep);file_path = [file_path filesep];end
...
% timeseries
time_eyelink   = double(BEHAVE.eyelink_time(1:min_length)');
time_tgt       = double(BEHAVE.t(1:min_length)');
...
event_state_combined(event_state_combined>5) = 5;
...
if isempty(time_photodiode_rise)
    str_fixation_rise   = ESN_Round( EPHYS.CH_EVE.data( (EPHYS.CH_EVE.data(:,2) == 1) & ((EPHYS.CH_EVE.data(:,3) == 1)) , 1) , 0.001);
    sac_detect_off_rise = ESN_Round( EPHYS.CH_EVE.data( (EPHYS.CH_EVE.data(:,2) == 3) & ((EPHYS.CH_EVE.data(:,3) == 1)) , 1) , 0.001);
...
if isempty(time_photodiode_fall)
    str_fixation_fall   = ESN_Round( EPHYS.CH_EVE.data( (EPHYS.CH_EVE.data(:,2) == 1) & ((EPHYS.CH_EVE.data(:,3) == 0)) , 1) , 0.001);
    sac_detect_off_fall = ESN_Round( EPHYS.CH_EVE.data( (EPHYS.CH_EVE.data(:,2) == 3) & ((EPHYS.CH_EVE.data(:,3) == 0)) , 1) , 0.001);
