function ESN_plot_double_pCells_v3
%% Load CH1 dataset
[file_name,file_path] = uigetfile([pwd filesep '*_sorted*.psort'], 'Select CH-1 file');
fprintf(['Loading ', file_name, ' as CH1 ... ']);
% EPHYS.CH_1_sorted = load([file_path filesep file_name], 'CH_data', 'SS_data', 'CS_data', 'Corr_data');
CH_1_DATA_PSORT = Psort_read_psort([file_path filesep file_name]);
EPHYS.CH_1_sorted = build_CH_sorted(CH_1_DATA_PSORT);
EPHYS.CH_1_sorted_file_name = file_name;
EPHYS.CH_1_sorted_file_path = file_path;
fprintf(' --> Completed. \n')

%% Load CH2 dataset
[file_name,file_path] = uigetfile([file_path filesep '*_sorted*.psort'], 'Select CH-2 file');
fprintf(['Loading ', file_name, ' as CH2 ... ']);
% EPHYS.CH_2_sorted = load([file_path filesep file_name], 'CH_data', 'SS_data', 'CS_data', 'Corr_data');
CH_2_DATA_PSORT = Psort_read_psort([file_path filesep file_name]);
EPHYS.CH_2_sorted = build_CH_sorted(CH_2_DATA_PSORT);
...
[save_file_name,save_file_path] = uiputfile([EPHYS.CH_1_sorted_file_path filesep file_name '.mat'], 'Select where to save the _sorted mat-file.');
...
save([save_file_path filesep save_file_name '.mat'], 'CH_1','CH_2','-v7.3');
...
%% function build CH_sorted from PSORT_DATA
function CH_sorted = build_CH_sorted(DATA_PSORT)
% EPHYS.CH_1_sorted.CH_data.CH_hipass
% EPHYS.CH_1_sorted.SS_data.SS_inds
% EPHYS.CH_1_sorted.SS_data.CS_inds
% EPHYS.CH_1_sorted.SS_data.SS_time
% EPHYS.CH_1_sorted.CS_data.CS_time

%% build CH_sorted from DATA_PSORT
...
CH_sorted.CH_data.CH_hipass  = ch_data;
CH_sorted.SS_data.SS_ind  = SS_index;
CH_sorted.SS_data.SS_inds = SS_inds;
CH_sorted.CS_data.CS_ind  = CS_index;
CH_sorted.CS_data.CS_inds = CS_inds;
CH_sorted.SS_data.SS_time = SS_time;
CH_sorted.CS_data.CS_time = CS_time;
CH_sorted.SS_data.SS_waveform = SS_waveform;
CH_sorted.CS_data.CS_waveform = CS_waveform;

%% build SSxSS AUTO PROBABILITY
SS_time   = CH_sorted.SS_data.SS_time;
CS_time   = CH_sorted.CS_data.CS_time;
Corr_data = ESN_correlogram(SS_time, CS_time);
CH_sorted.Corr_data.Corr_CS_inds_span     = Corr_data.CS_inds_span;
CH_sorted.Corr_data.Corr_CS_bin_size_time = Corr_data.CS_bin_size_time;
CH_sorted.Corr_data.Corr_SS_inds_span     = Corr_data.SS_inds_span;
CH_sorted.Corr_data.Corr_SS_bin_size_time = Corr_data.SS_bin_size_time;
CH_sorted.Corr_data.Corr_SS_SSxSS_AUTO    = Corr_data.SS_SSxSS_AUTO;
CH_sorted.Corr_data.Corr_CS_CSxSS_AUTO    = Corr_data.CS_CSxSS_AUTO;
end