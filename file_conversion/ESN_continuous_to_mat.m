...
% mkdir('kwik_files')
% convert_open_ephys_to_kwik([pwd filesep], [pwd filesep 'kwik_files'])
...
FILE_NAMES_CONTINUOUS = dir([folder_path '*.continuous']);
mkdir([folder_path filesep 'mat_files'])
% session_info = get_session_info(folder_path);
%%
counter_CH_CMN = 0;
for counter_file = 1 : length(FILE_NAMES_CONTINUOUS)
    file_info = FILE_NAMES_CONTINUOUS(counter_file);
    [~,file_name,~] = fileparts(file_info.name);
    file_path_name = [folder_path file_info.name];
    
    fprintf([num2str(counter_file) '/' num2str(length(FILE_NAMES_CONTINUOUS)) '. Loading: ' file_name ' ... '])
    
    clearvars('ch_data', 'ch_time', 'ch_info');
    [ch_data, ch_time, ch_info] = load_open_ephys_data(file_path_name);
    clearvars('CH_DATA');
    CH_DATA.ch_data = ch_data;
    CH_DATA.ch_time = ch_time;
    CH_DATA.ch_info = ch_info;
    save([folder_path filesep 'mat_files' filesep file_name '.mat'], 'CH_DATA', '-v7.3');
    
    if (contains(file_name,'_CH'))
...
port_num_string = file_name(1:3);

[ch_data, ch_time, ch_info] = load_open_ephys_data([folder_path 'all_channels.events']);
clearvars('CH_DATA');
CH_DATA.ch_data = ch_data;
CH_DATA.ch_time = ch_time;
CH_DATA.ch_info = ch_info;
file_name = [port_num_string '_EVE1'];
save([folder_path filesep 'mat_files' filesep file_name '.mat'], 'CH_DATA', '-v7.3');

CH_CMN_data = CH_CMN_data ./ counter_CH_CMN;
clearvars('CH_DATA');
CH_DATA.ch_data = CH_CMN_data;
CH_DATA.ch_time = CH_CMN_time;
CH_DATA.ch_info = CH_CMN_info;
file_name = [port_num_string '_CMN1'];
save([folder_path filesep 'mat_files' filesep file_name '.mat'], 'CH_DATA', '-v7.3');
